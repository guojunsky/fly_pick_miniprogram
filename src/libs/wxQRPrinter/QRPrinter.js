...
let glbCmd = ""
let maxBit = 200
let gbToBase64 = require("./libs/gb2312ToBase64.js").encode64
let _currentBold = -1
let _currentFont = -1
...
function deviceFilter(devices){
  ...
  for (let i = 0; i < devices.length; i++) {
    ...
    console.log(device,'deviceFilter')
    // if ((device.localName && device.localName.toUpperCase().startsWith("QR"))){
    //   printers.push({
    //     name:device.name,
    ...
    success: function (res) {
      ...
      wx.getBLEDeviceCharacteristics({
        ...
        success: function (res) {
          ...
          var readId;
          var writeId;
          console.log('getBLEDeviceCharacteristics', connectedDeviceId, serviceId, res);
          for (let i = 0; i < res.characteristics.length; i++) {
            ...
            if ((notify || indicate) && write){
                writeCharacteristic = writeId;
                readCharacteristic = readId;
            }
            ...
            // // }
            // if (res.characteristics[i].uuid.indexOf('49535343-8841-43F4-A8D4-ECBE34729BB3') != -1 ||res.characteristics[i].uuid.indexOf('49535343-aca3-481c-91ec-d85e28a60318') != -1 || res.characteristics[i].uuid.indexOf('49535343-ACA3-481C-91EC-D85E28A60318') != -1) {
...
function sendCmd(cmd , print, callback){
  glbCmd = glbCmd + cmd + "\n\r"
  if(!print)
  {
    return
  ...
  writeBLEValue(arrayBuffer, callback)
  
}
function writeBLEValue(arrayBuffer, callback){
  console.log(serviceId,'serviceId');
  console.log(writeCharacteristic,'writeCharacteristic');
  ...
  console.log(count,'writeBLEValue_count')
  if (count > maxBit) 
  {
    ...
    try{
      ...
      ({
        ...
        success: function (res) {
          // console.log("分部传输中")
          writeBLEValue(leftArrayBuffer, callback)
        
        },
        fail:function(e){
          callback && callback('fail')
          console.log(e,'writeBLEValue_e')
          wx.hideLoading()
          wx.showToast({
            title: "传输数据失败",
            image: "src/icon_fail.png"
...
*/
function print(revervse, skip, callback) {
  if (skip) feed();
  if (!revervse) {
    let cmd = "PRINT";
    sendCmd(cmd, true, callback);
  }
  else {
    let cmd = "POPRINT";
    sendCmd(cmd, true, callback);
