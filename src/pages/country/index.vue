	...
	<view class="content">
		<view class="search_box">
			<view class="s_box">
				...
				<input class="inp" type="text" placeholder="国家中文名/拼音" v-model="scity" @input="handlerSearch" />
			</view>
		</view>
		<!-- list -->
		<view class="list">
			...
			<scroll-view class="list_content" scroll-y="true" :scroll-into-view="viewId">
				<view class="hot_content" v-if="hotList.length">
					<view class="hot_title">热门国家</view>
					<view class="hot_wrap">
						<view class="hot_item" v-for="(item,index) in hotList" :key="item.id" @click="handlerCity(item)">{{item.countryTwoCode}}{{item.cnName}}</view>
					</view>
				</view>
				<template v-for="(item,index) in cityWords">
					<view class="title" :id="item">{{item}}</view>
					<view class="city_list">
						<view class="city_name" v-for="(citem,cindex) in cityInners[item]" :key="citem.id" @click="handlerCity(citem)">{{citem.countryTwoCode}}{{citem.cnName}}</view>
					</view>
				</template>
			</scroll-view>
		...
		methods: {
			...
			makeData(data) {
				...
				data.forEach((item, index) => {
					const hot = item.hot;
					const enName = item.countryTwoCode.toUpperCase().substring(0, 1)
					if (hot) {
						hotList.push(item)
					}
					if (!cityInners[enName]) {
						cityInners[enName] = []
						cityWords.push(enName)
					}
					cityInners[enName].push(item)
				})
			...
			handlerSearch(event) {
				...
				if (value) {
					citys.forEach((item, index) => {
						...
						if (cnName.indexOf(value) > -1 || enName.indexOf(value) > -1 || (py && py.indexOf(cvalue) > -1) ||  (shortPy && shortPy.indexOf(cvalue) > -1)) {
							list.push(item)
						}
					})
					this.makeData(list)
			...
			handlerCity(item) {
				this.$store.dispatch('app/toggleCountry', item)
				// 全局事件
				uni.$emit(chooseCountry, item)
				uni.navigateBack()
			}
		}
	}
