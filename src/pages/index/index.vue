	...
	<view class="content">
		...
		<view class="wrap">
			<view class="tab_content">
				<view v-for="(item,index) in tablist" :class="['tab_item',activeIndex == item.value ? 'active' : '']" :key="index"
			...
			<view class="send_box">
				...
				<view class="send_wrap">
					...
					<view class="detail_place">
						<view :class="[city ? 'from' : 'fromnone']" @click="handlerCityPage">{{city ? city.cityName : '城市'}}</view>
						<image class="center_arrow" src="/static/index/right_arrow.png"></image>
						<view :class="[country ? 'to' : 'fromnone']" @click="handlerCountryPage">{{country ? country.cnName : '国家'}}</view>
					</view>
					<view class="look_content" v-if="activeIndex !== 1">
						<text class="only_txt">只看</text>
						<view class="only_item" v-for="(item,index) in typeObj[activeIndex]" :key="index" @click="handlerOnlyItem(item)">
							<image class="only_img" v-if="inArray(onlyCheck[activeIndex],item.value)" src="../../static/index/checked.png"></image>
					...
					<view class="history" v-if="history.length">
						<view class="history_list">
							<text class="h_txt" v-for="(item,index) in history" :key="index" @click="handlerHisroty(item)">{{item.city.cityName}}-{{item.country.cnName}}</text>
						</view>
		...
		computed: {
			...
			goodsNature() {
				if (this.activeIndex == 1 || this.activeIndex == 2) {
					...
					const result = this.onlyCheck[3].length ? (this.onlyCheck[3].length > 1 ? [4] : this.onlyCheck[3]) : [3];
					return result
				}
			},
			dispatcherTypes() {
