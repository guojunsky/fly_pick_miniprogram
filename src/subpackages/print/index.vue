<template>
  <div>
    <div :class="['page', 'page-grey']">
      ...
      <div class="page_bd">
        ...
        <div class="order_tt">该订单包含{{ goodsItems.length }}件货品（每件打印一张面单）</div>
         ...
         <div class="order_item" v-for="(item,index) in goodsItems" :key="index" @click="changeGoodsItemSelected(item,index)">
    ...
    <canvas style="width: 160px; height: 160px; visibility: hidden" v-if="showCanvas" canvas-id="qrcode"
            id="qrcode"></canvas>
    <canvas style="width: 300px; height: 50px; visibility: hidden" v-if="showCanvas" canvas-id="barcode"
            id="barcode"></canvas>
  </div>
...
export default {
  ...
  computed: {
    ...
    goodsItems() {
      return this.$store.state.print.printOrder && this.$store.state.print.printOrder.goods && this.$store.state.print.printOrder.goods.items;
    },
    selectedNum() {
  ...
  onLoad(option) {
    ...
    if (option.deviceId) {
      ...
      this.deviceConnect(this.deviceId);
    } else {
      ...
      QRPrinter.searchPrinter(function(devices){
         that.deviceConnect(this.deviceId);
      });
    }
  ...
  methods: {
    ...
    write180Model(item, orderInd){
      return new Promise((resolve, reject)=>{
          ...
          QRPrinter.drawText(x + 24, y + 100, strReset(productNo), 3, 0, 1, false, false)
    ...
    async onConnectReady(item, index, resolve) {
      ...
      let params = {
        ...
        productNo: printOrder.productNo//产品编号
      }
